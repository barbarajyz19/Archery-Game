...
## Mise en place du TP
  ...
  1. Dans le dossier `Archery-Game/` exécutez
```bash  	  
Archery-Game$  npm install
```  
...
```bash
Archery-Game$  npm run dev-server
```
...
Pour executer le projet il faut avoir fais les commandes suivantes en se placant dans le dosser tp3/:

```	  
npm install
npm run build (pour créer le dossier `./dist/` et construire le bundle )
npm run dev-server (ouvre le projet)
npm run build (une fois terminer)
``` 
## Travail réalisé
